...
if ( !defined( 'PEXETO_UPDATE_XML_FILE' ) )
	define( 'PEXETO_UPDATE_XML_FILE', 'http://pexeto.com/updates/photolux.xml' );
if ( !defined( 'PEXETO_DOCUMENTATION_URL' ) )
	define( 'PEXETO_DOCUMENTATION_URL', 'http://pexetothemes.com/docs/photolux/#updates' );
...
if ( !function_exists( 'pexeto_init_update_notfier' ) ) {
	function pexeto_init_update_notfier() {
		...
		if ( is_admin() && $pagenow!='update-core.php' ) {
			$update_notifier = new PexetoUpdateNotifier(
				PEXETO_THEMENAME,
				PEXETO_SHORTNAME,
				...
				PEXETO_DOCUMENTATION_URL
			);
			$update_notifier->init();
		}
	}
}
...
function pexeto_admin_init(){
	...
	if(isset($_GET['page']) && $_GET['page']==PEXETO_OPTIONS_PAGE){
		...
		wp_enqueue_style('pexeto-admin-style',PEXETO_CSS_URL.'admin_style.css', array(), PEXETO_VERSION);
		wp_enqueue_style('pexeto-colorpicker-style',PEXETO_CSS_URL.'colorpicker.css');
	...
	if(isset($_GET['page']) && ($_GET['page']==PEXETO_UPDATE_PAGE_NAME)){
